...
function convertToNumber(str) {
    const cleanStr = str.split("").filter((ch)=>ch >= "0" && ch <= "9" || ch === ".").join("");
...
function sortList(columnIndex, isReverse) {
    rows.sort((rowA, rowB)=>{
        let value1 = rowA.querySelector(`td:nth-child(${columnIndex + 1})`).textContent;
        let value2 = rowB.querySelector(`td:nth-child(${columnIndex + 1})`).textContent;
        if (columnIndex === 3 || columnIndex === 4) {
            value1 = convertToNumber(value1);
            value2 = convertToNumber(value2);
        }
        if (isReverse) [value1, value2] = [
            value2,
            value1
        ];
        return typeof value1 === "string" && typeof value2 === "string" ? value1.localeCompare(value2) : value1 - value2;
    });
...
function pushSuccessNotification() {
    return pushNotification("success", "Congratulation!", "Employee added successfully!");
}
...
form.addEventListener("submit", (ev)=>{
    ...
    const salaryToStr = salary.toString().split("").reverse().map((num, index)=>index % 3 === 0 && index !== 0 ? `${num},` : num).reverse().join("");
...
tbody.addEventListener("dblclick", (eve)=>{
    ...
    if (cellIndex === 4 || cellIndex === 5) newInput.setAttribute("type", "number");
    ...
    newInput.addEventListener("blur", ()=>{
        ...
        if (cellIndex === 5) newValue = `$${newInput.value.toString().split("").reverse().map((num, index)=>index % 3 === 0 && index !== 0 ? `${num},` : num).reverse().join("")}`;
        cell.textContent = newValue === "" ? currentValue : newValue;
    });
    newInput.addEventListener("keydown", (k)=>{
        if (k.key === "Enter") newInput.blur();
    });
    newInput.focus();
});
